...
export default function ScannerPage() {
  ...
  useEffect(() => {
    ...
    // Parse ?member= from URL (from QR scan)
  ...
  async function lookupMember(id: string) {
    ...
    const { data, error: dbError } = await supabase
      ...
      .select("id, name, membership_type, slip_status, sessions_remaining, kids_names, kids_count")
      .eq("id", id)
      .single();
  ...
  return (
    <div className="min-h-dvh bg-gray-900 flex flex-col items-center justify-start pt-10 px-4">
      ...
      {/* Success check-in */}
      {checkedIn && member && (
        <div className="bg-green-500 text-white rounded-2xl p-6 text-center max-w-sm w-full animate-pulse">
          <div className="text-5xl mb-3">✓</div>
          <h2 className="font-fredoka text-3xl mb-1">{member.name}</h2>
          <p className="text-green-100">Checked in successfully!</p>
        </div>
      )}
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full">
          ...
          {member.slip_status !== "approved" && (
            <div className="bg-yellow-100 text-yellow-800 rounded-xl px-3 py-2 mb-4 text-sm flex items-center gap-2">
              <span>⚠️</span>
              <span>Payment not approved — check with member</span>
            </div>
          )}
          ...
          {member.sessions_remaining !== null && (
            <div className="bg-blue-50 rounded-xl px-3 py-2 my-3 text-center">
              ...
              <p className="font-fredoka text-3xl text-[#1a56db]">{member.sessions_remaining}</p>
            </div>
          )}
      ...
      {!member && !loading && !error && (
        <div className="text-gray-600 text-center text-sm mt-8 max-w-xs">
